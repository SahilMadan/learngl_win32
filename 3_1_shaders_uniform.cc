...
constexpr double kFramesPerSecond = 60.0f;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  const long long milliseconds_per_frame = 16;
  while (msg.message != WM_QUIT) {
    if (PeekMessage(&msg, /*hWnd=*/NULL, /*wMsgFilterMin=*/0,
                    /*wMsgFilterMax=*/0, PM_REMOVE)) {
      ...
      elapsed_milliseconds.QuadPart = (current.QuadPart - previous.QuadPart);

      // We now have the number of elapsed number of ticks along with ticks per
      // second (frequency). Convert to milliseconds *before* dividing by ticks
      // per second.
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam,
  ...
  case WM_PAINT: {
    ...
    const double time = (double)GetTickCount() / 1000.0;
